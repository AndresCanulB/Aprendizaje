...
// Lista de palabras que se pueden usar en el juego
...
// Esta función mezcla las letras para que la palabra quede desordenada
...
const TareaScrambleWords = () => {
  // Palabras de la lista GAME_WORDS para el juego
  const [words, setWords] = useState(shuffleArray(GAME_WORDS));

  // Palabra a adivinar que se muestra en pantalla
  const [currentWord, setCurrentWord] = useState(words[0]);

  // La misma palabra desordenada a adivinar que se muestra en pantalla
  const [scrambledWord, setScrambledWord] = useState(scrambleWord(currentWord));

  // El texto input que ingresa el usuario para adivinar la palabra
  const [guess, setGuess] = useState("");

  // Aciertos de palabras correctas adivinadas por el usuario
  const [points, setPoints] = useState(0);

  // Errores de palabras incorrectas que el usuario no pudo adivinar
  const [errorCounter, setErrorCounter] = useState(0);

  // Numero maximo de errores que puede tener un usuario antes de que acabe el juego
  const [maxAllowErrors, setMaxAllowErrors] = useState(3);

  // Numero de veces que el usuario a saltado una palabra que no sabia
  const [skipCounter, setSkipCounter] = useState(0);

  // Maximo numero de veces que el usuario puede saltar las palabras que no sabe
  const [maxSkips, setMaxSkips] = useState(3);

  // Booleano que define el fin del juego
  const [isGameOver, setIsGameOver] = useState(false);

  // useState actualiza el estado asincronicamente, por esto puede dar errores en las comprobaciones
  // El useEffect se ejecuta despues de que useState haya cambiado los estados al final.

  // useEffect para agregar una nueva palabra a la pantalla
  useEffect(() => {
  ...
  // useEffect para verificar que se termino el juego
  useEffect(() => {
    if (errorCounter === maxAllowErrors) setIsGameOver(true);
  }, [errorCounter, maxAllowErrors]);

  // Funcion que debe verificar que el texto del input sea correcto en base a la palabra en pantalla
  const handleGuessSubmit = (e: React.FormEvent) => {
    // Previene el refresh de la página
    e.preventDefault();

    if (guess !== currentWord) setErrorCounter(errorCounter + 1);
  ...
  // Funcion que debe saltar la palabra en pantalla y debe mostrar otra palabra
  const handleSkip = () => {
  ...
  // Funcion que despues de que termine el juego debe empezar de nuevo el juego
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-100 via-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="w-full max-w-md mx-auto">
          <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent mb-2">Palabras desordenadas</h1>
...
export default TareaScrambleWords;
