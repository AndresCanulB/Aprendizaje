// Cuando se importan interfaces o tipos se recomienda agregar la palabra reservada "type"
...
export function getHeroesByOwner(owner: Owner): Hero[] {
  const heroesOwner = heroes.filter((hero) => hero.owner === owner);
  return heroesOwner;
}
